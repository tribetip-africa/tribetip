...
TIPPABLE_USERNAME="${TIPPABLE_USERNAME:-$CREATOR_USERNAME}"
K6_SCRIPT="${K6_SCRIPT:-k6-mixed-write.js}"
...
echo "Running k6 (${K6_SCRIPT}) on network ${NETWORK}:"
echo "  reads  ${READ_RPM} RPM (~$((READ_RPM / 60))/s)"
echo "  writes ${WRITE_RPM} RPM (~$((WRITE_RPM / 60))/s)"
