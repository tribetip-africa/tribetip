...
module Tribetip
  module Referrals
    class Attach
      def self.call(referred:, referral_code:, attach_ip: nil)
        new(referred: referred, referral_code: referral_code, attach_ip: attach_ip).call
      end

      def initialize(referred:, referral_code:, attach_ip: nil)
        @referred = referred
        @referral_code = Referrals.normalize_input(referral_code)
        @attach_ip = attach_ip.to_s.strip.presence
      end
      ...
      def call
        ...
        return if FraudCheck.attach_blocked?(referrer: referrer, attach_ip: @attach_ip)
        ...
        ActiveRecord::Base.transaction do
          ...
          Referral.create!(
            ...
            metadata: {
              ...
              attach_ip: @attach_ip,
              # Kept for older qualification fraud checks reading signup_ip.
              signup_ip: @attach_ip,
