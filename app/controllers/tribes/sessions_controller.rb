module Tribes
  class SessionsController < Devise::SessionsController
    ...
    def reject_suspended_sign_in
      login = params.dig(:tribe, :login)&.to_s&.strip&.downcase
      password = params.dig(:tribe, :password)
      return if login.blank? || password.blank?

      tribe = Tribe.find_for_database_authentication(login: login)
      return unless tribe&.valid_password?(password)
